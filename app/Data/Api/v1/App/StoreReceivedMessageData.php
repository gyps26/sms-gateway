...
class StoreReceivedMessageData extends BaseData
{
    ...
    public static function rules(ValidationContext $context, #[RouteParameter('device')] Device $device): array
    {
        return [
            'content' => [
                Rule::when(
                    data_get($context->fullPayload, 'type') === 'MMS',
                    ['nullable', 'required_without:attachments'],
                    ['required']
                ),
                'string'
            ],
            'sim_slot' => [
                'required',
                new In($device->sims->pluck('slot')->toArray())
            ],
            'attachments.*' => ['file'],
        ];
    }
