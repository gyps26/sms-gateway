...
class ContactListPolicy
{
    ...
    {
        $count += $user->contactLists()->count();

        return $user->canConsume($count, 'contact_lists');
    }
    ...
    public function export(User $user, ContactList $contactList): Response|bool
    {
        if ($contactList->user_id === $user->id) {
