<?php
/*
 * Copyright © 2018-2025 RBSoft (Ravi Patel). All rights reserved.
 *
 * Author: Ravi Patel
 * Website: https://rbsoft.org/downloads/sms-gateway
 *
 * This software is licensed, not sold. Buyers are granted a limited, non-transferable license
 * to use this software exclusively on a single domain, subdomain, or computer. Usage on
 * multiple domains, subdomains, or computers requires the purchase of additional licenses.
 *
 * Redistribution, resale, sublicensing, or sharing of the source code, in whole or in part,
 * is strictly prohibited. Modification (except for personal use by the licensee), reverse engineering,
 * or creating derivative works based on this software is strictly prohibited.
 *
 * Unauthorized use, reproduction, or distribution of this software may result in severe civil
 * and criminal penalties and will be prosecuted to the fullest extent of the law.
 *
 * For licensing inquiries or support, please visit https://support.rbsoft.org.
 */

namespace App\Observers;

use App\Enums\CampaignStatus;
use App\Enums\CampaignType;
use App\Jobs\AutoRetryCampaign;
use App\Models\Campaign;
use App\Models\Setting;
use App\Monitor;

class CampaignObserver
{
    /**
     * Handle the Campaign "created" event.
     */
    public function created(Campaign $campaign): void
    {
        //
    }

    /**
     * Handle the Campaign "updated" event.
     */
    public function updated(Campaign $campaign): void
    {
        if ($campaign->status === CampaignStatus::Completed && $campaign->type !== CampaignType::UssdPull) {
            if (config('messaging.auto_retry.enabled') && Setting::retrieve('messaging.auto_retry.enabled', $campaign->user_id)) {
                AutoRetryCampaign::dispatch($campaign);
            }
        }
    }

    /**
     * Handle the Campaign "deleting" event.
     */
    public function deleting(Campaign $campaign): void
    {
        if ($campaign->status !== CampaignStatus::Processed) {
            Monitor::for(Campaign::class, $campaign->id)->cancel();
        }
    }

    /**
     * Handle the Campaign "deleted" event.
     */
    public function deleted(Campaign $campaign): void
    {
        //
    }

    /**
     * Handle the Campaign "restored" event.
     */
    public function restored(Campaign $campaign): void
    {
        //
    }

    /**
     * Handle the Campaign "force deleted" event.
     */
    public function forceDeleted(Campaign $campaign): void
    {
        //
    }
}
